...
probe_sepolia_rpc_capabilities() {
  ...
  latest_hex=$(cast rpc --rpc-url "$rpc_url" eth_blockNumber | tr -d '"')
  ...
  block=$(cast rpc --rpc-url "$rpc_url" eth_getBlockByNumber "$historical_hex" false)
  if ! jq -e --arg number "${historical_hex,,}" \
  ...
  historical_call=$(
    cast rpc --rpc-url "$rpc_url" --raw eth_call \
  ...
  logs=$(
    cast rpc --rpc-url "$rpc_url" --raw eth_getLogs \
  ...
  receipt_block=$(cast rpc --rpc-url "$rpc_url" eth_getBlockByNumber latest false)
  transaction_hash=$(jq -r '.transactions[0] // empty' <<<"$receipt_block")
  for offset in $(seq 1 32); do
    [[ -n "$transaction_hash" ]] && break
    receipt_block_decimal=$((latest_decimal - offset))
    printf -v receipt_block_hex '0x%x' "$receipt_block_decimal"
    receipt_block=$(cast rpc --rpc-url "$rpc_url" eth_getBlockByNumber "$receipt_block_hex" false)
    transaction_hash=$(jq -r '.transactions[0] // empty' <<<"$receipt_block")
  done
  ...
  receipt=$(cast rpc --rpc-url "$rpc_url" eth_getTransactionReceipt "$transaction_hash")
  if ! jq -e --arg hash "${transaction_hash,,}" \
  ...
  blob_base_fee=$(cast rpc --rpc-url "$rpc_url" eth_blobBaseFee | tr -d '"')
